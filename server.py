...
"""
K线训练系统 - 使用离线数据
支持 Qlib 格式和 baostock 下载格式，自动识别
配置路径：config.json
"""
import os
import json
...
def load_config():
    ...
    default = {
        "data_dirs": [
            os.path.expanduser("~/.qlib/qlib_data/cn_data/stock"),
            os.path.join(os.path.dirname(__file__), "data", "daily")
        ],
        "port": 8889
    }
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, 'r') as f:
                cfg = json.load(f)
            # 展开 ~ 路径
            cfg["data_dirs"] = [os.path.expanduser(d) for d in cfg.get("data_dirs", default["data_dirs"])]
            cfg["port"] = cfg.get("port", default["port"])
            return cfg
        except Exception:
            pass
    return default
...
def detect_format(data_dir):
    """识别数据目录格式：qlib 或 baostock"""
    # Qlib格式：{data_dir}/{sh|sz}/{code}/{code}.csv
    for m in ['sh', 'sz']:
        mdir = os.path.join(data_dir, m)
        if os.path.isdir(mdir):
            for code in os.listdir(mdir):
                csv = os.path.join(mdir, code, f"{code}.csv")
                if os.path.isfile(csv):
                    return 'qlib'
    # baostock格式：{data_dir}/{code}.csv
    for f in os.listdir(data_dir):
        if f.endswith('.csv'):
            return 'baostock'
    return None

def normalize_df(df, fmt):
    """统一列名，返回标准化的 DataFrame"""
    if fmt == 'baostock':
        # baostock下载格式：date,code,open,high,low,close,volume,amount,turn,pctChg
        col_map = {'turn': 'turnover', 'pctChg': 'pct_chg'}
        ...
        if fmt == 'qlib':
            for market in ['sh', 'sz']:
                ...
                for code in os.listdir(market_dir):
                    ...
                    if os.path.isfile(csv_path) and full_code not in seen:
                        seen.add(full_code)
                        stocks.append({"code": full_code, "market": market, "name": code})
        elif fmt == 'baostock':
            for f in os.listdir(data_dir):
                if not f.endswith('.csv'):
                    continue
                code = f.replace('.csv', '')
                # 尝试识别 sh.600000 或 600000.SH 格式
                if '.' in code:
                    parts = code.split('.')
                    if parts[0].lower() in ('sh', 'sz'):
                        full_code = parts[0].lower() + '.' + parts[1]
                    else:
                        full_code = parts[1].lower() + '.' + parts[0]
                else:
                    continue
                csv_path = os.path.join(data_dir, f)
                market = full_code.split('.')[0]
...
def find_csv_path(stock_code):
    """查找股票CSV文件路径，遍历所有数据目录"""
    ...
    for data_dir in DATA_DIRS:
        if not os.path.isdir(data_dir):
            continue
        # Qlib格式
        qlib_path = os.path.join(data_dir, market, code, f"{code}.csv")
        if os.path.isfile(qlib_path):
            return qlib_path
        # baostock格式
        for fname in [f"{stock_code}.csv", f"{code}.{market.upper()}.csv"]:
            bs_path = os.path.join(data_dir, fname)
...
def load_stock_data(code):
    """加载股票日线数据"""
    csv_path = find_csv_path(code)
    if not csv_path:
        return None
    df = pd.read_csv(csv_path)
    df = normalize_df(df, detect_format(os.path.dirname(csv_path)) if 'daily' in csv_path else 'qlib')
...
def get_training_data(code, start_idx, length):
    ...
    for _, row in result.iterrows():
        records.append({
            ...
            "ma5": r2(row['ma5']), "ma10": r2(row['ma10']),
            "ma20": r2(row['ma20']), "ma30": r2(row['ma30']),
            "macd_dif": r2(row['macd_dif']), "macd_dea": r2(row['macd_dea']),
            "macd_bar": r2(row['macd_bar']),
            "kdj_k": r2(row['kdj_k']), "kdj_d": r2(row['kdj_d']),
            "kdj_j": r2(row['kdj_j']),
            "dma": r2(row['dma']), "dma_ama": r2(row['dma_ama'])
        })
    return records
...
def get_random_training(length, stock_code=None):
    ...
    for _ in range(10):
        ...
        return {
            "code": stock['code'],
            "klines": klines, "answer": answer,
            "start_idx": start_idx, "total_len": len(df),
...
def get_stock_next_training(stock_code, start_idx, length):
    ...
    return {
        "code": stock_code,
        "klines": klines, "answer": answer,
        "start_idx": start_idx, "next_idx": start_idx + 1,
        "total_len": len(df), "buy_hold_return": buy_hold_return
    }
...
def get_stock_full_data(stock_code, length):
    ...
    if not klines:
        return None
    return {"code": stock_code, "klines": klines, "total": len(klines)}
